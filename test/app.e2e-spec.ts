...
describe('Admin JSON Web Token (e2e)', () => {
  ...
  it('/admin/books (GET) - with valid token', () => {
    return request(app.getHttpServer())
      .get('/admin/books') // Try hitting admin find all books API
  ...
  it('/admin/books (GET) - with invalid token', () => {
    return request(app.getHttpServer())
      .get('/admin/books') // Try hitting admin find all books API
  ...
  it('/user/books (GET) - with valid token', () => {
    return request(app.getHttpServer())
      .get('/user/books') // Try hitting user find all books API
      .set('Authorization', `Bearer ${accessToken}`)
...
describe('User JSON Web Token (e2e)', () => {
  ...
  it('/user/books (GET) - with invalid token', () => {
    return request(app.getHttpServer())
      .get('/user/books') // Try hitting user find all books API
      .set('Authorization', 'Bearer invalidToken')
      .expect(401); // Expect unauthorized response from hitting user API
